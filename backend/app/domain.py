...
DOMAIN_SPECS: list[EntitySpec] = [
    ...
    EntitySpec(
        operations.AssetAssignment, "asset-assignment", "Asset Assignment", "finance_accounting",
        kind="transaction", icon="users", search_fields=["location_name", "remarks"],
        fields=[
            _f("item_id", "Item", "reference", required=True, reference_entity="inventory-item"),
            _f("assignee_type", "Assignee Type", "select", options_master="asset_assignee_type"),
            _f("student_id", "Student", "reference", reference_entity="student"),
            _f("employee_id", "Employee", "reference", reference_entity="employee"),
            _f("location_name", "Location"),
            _f("quantity", "Quantity", "number", required=True),
            _f("issue_date", "Issue Date", "date"),
            _f("due_date", "Due Date", "date"),
            _f("return_date", "Return Date", "date"),
            _f("assignment_status", "Status", "select", options_master="asset_assignment_status"),
            _f("remarks", "Remarks", "text", list_visible=False),
        ],
    ),
    # ----------------------------------------------------------------- Activities & Events
    EntitySpec(
        operations.Activity, "activity", "Activity / Sport", "activities_events",
        kind="master", icon="trending-up", search_fields=["name", "code"],
        fields=[
            _f("name", "Name", required=True),
            _f("code", "Code", required=True),
            _f("activity_type", "Type", "select", options_master="activity_type"),
            _f("in_charge_id", "In-charge (Teacher/Staff)", "reference", reference_entity="employee"),
            _f("venue", "Venue"),
            _f("schedule", "Schedule"),
            _f("start_date", "Start Date", "date"),
            _f("fee", "Fee", "decimal"),
            _f("capacity", "Capacity", "number"),
            _f("activity_status", "Status", "select", options_master="activity_status"),
        ],
    ),
    EntitySpec(
        ...
        fields=[
            ...
            _f("amount", "Amount", "decimal"),
            _f("payment_status", "Payment", "select", options_master="reg_payment_status"),
            _f("paid_at", "Paid On", "date"),
        ],
    ),
    EntitySpec(
        operations.Facility, "facility", "Facility", "activities_events",
        kind="master", icon="grid", search_fields=["name", "code", "location"],
        fields=[
            _f("name", "Name", required=True),
            _f("code", "Code", required=True),
            _f("facility_type", "Type", "select", options_master="facility_type"),
            _f("in_charge_id", "In-charge (Teacher/Staff)", "reference", reference_entity="employee"),
            _f("location", "Location"),
            _f("capacity", "Capacity", "number"),
            _f("usage_fee", "Usage Fee", "decimal"),
            _f("facility_status", "Status", "select", options_master="facility_status"),
            _f("description", "Description", "text"),
        ],
    ),
    EntitySpec(
        operations.FacilityBooking, "facility-booking", "Facility Booking", "activities_events",
        kind="transaction", icon="calendar", search_fields=["purpose"],
        fields=[
            _f("facility_id", "Facility", "reference", required=True, reference_entity="facility"),
            _f("student_id", "Student", "reference", reference_entity="student"),
            _f("booking_date", "Date", "date"),
            _f("slot", "Slot"),
            _f("purpose", "Purpose"),
            _f("amount", "Amount", "decimal"),
            _f("payment_status", "Payment", "select", options_master="reg_payment_status"),
            _f("booking_status", "Status", "select", options_master="booking_status"),
        ],
    ),
    # ----------------------------------------------------------------- Meal & Cafeteria
    EntitySpec(
    ...
    # ----------------------------------------------------------------- Teacher management
    EntitySpec(
        people.TeacherProfile, "teacher-profile", "Teacher Profile", "teacher_management",
        kind="master", icon="users", search_fields=["expertise", "certifications", "subjects_can_teach"],
        fields=[
            _f("employee_id", "Teacher", "reference", required=True, reference_entity="employee"),
            _f("expertise", "Expertise", "text"),
            _f("certifications", "Certifications", "text"),
            _f("subjects_can_teach", "Subjects Can Teach", "text"),
            _f("qualification", "Qualification"),
            _f("reporting_manager_id", "Reporting Manager", "reference", reference_entity="employee"),
        ],
    ),
    EntitySpec(
        people.TeacherAssignment, "teacher-assignment", "Teacher Assignment", "teacher_management",
        kind="transaction", icon="table", search_fields=[],
        fields=[
            _f("employee_id", "Teacher", "reference", required=True, reference_entity="employee"),
            _f("academic_year_id", "Academic Year", "reference", required=True, reference_entity="academic-year"),
            _f("grade_id", "Grade", "reference", required=True, reference_entity="grade"),
            _f("section_id", "Section", "reference", required=True, reference_entity="section"),
            _f("subject_id", "Subject", "reference", required=True, reference_entity="subject"),
            _f("reporting_manager_id", "HOD / Marks Approver", "reference", required=True, reference_entity="employee"),
            _f("effective_from", "From", "date"),
            _f("effective_to", "To", "date"),
            _f("assignment_status", "Status", "select", options_master="active_status"),
        ],
    ),
    # ----------------------------------------------------------------- Exam schedules and approval workflow
    EntitySpec(
        exams.ExamSubject, "exam-subject", "Exam Schedule", "examination_management",
        kind="transaction", icon="calendar", search_fields=["room"],
        fields=[
            _f("exam_id", "Exam", "reference", required=True, reference_entity="exam"),
            _f("subject_id", "Subject", "reference", required=True, reference_entity="subject"),
            _f("grade_id", "Grade", "reference", reference_entity="grade"),
            _f("section_id", "Section", "reference", reference_entity="section"),
            _f("assigned_teacher_id", "Teacher", "reference", reference_entity="employee"),
            _f("exam_date", "Date", "date"),
            _f("start_time", "Start", "time"),
            _f("end_time", "End", "time"),
            _f("room", "Room"),
            _f("max_marks", "Max Marks", "decimal"),
            _f("pass_marks", "Pass Marks", "decimal"),
            _f("schedule_status", "Status", "select", options_master="exam_schedule_status"),
        ],
    ),
    EntitySpec(
        exams.MarksBatch, "marks-batch", "Marks Batch", "examination_management",
        kind="transaction", icon="check-square", search_fields=["review_note"],
        fields=[
            _f("exam_id", "Exam", "reference", required=True, reference_entity="exam"),
            _f("subject_id", "Subject", "reference", required=True, reference_entity="subject"),
            _f("grade_id", "Grade", "reference", reference_entity="grade"),
            _f("section_id", "Section", "reference", reference_entity="section"),
            _f("teacher_id", "Teacher", "reference", reference_entity="employee"),
            _f("reviewer_id", "Reviewer", "reference", reference_entity="employee"),
            _f("batch_status", "Status", "select", options_master="marks_batch_status"),
            _f("review_note", "Review Note", "text", list_visible=False),
        ],
    ),
]
